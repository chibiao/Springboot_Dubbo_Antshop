$(function () {
        ...
        columns: [[
            {
                field: 'image', title: '商品图片', width: 100, align: 'center', formatter: function (value, row, index) {
                    return "<img src='" + row.image + "' style=width:80px;height:80px>";
                }
            },
    ...
    $("#del").click(function () {
        ...
        $.messager.confirm("确认", "是否做下架操作", function (res) {
            if (res) {
                $.ajax({
                    type:"put",
                    url:"/product/updateProductState/"+rowData.id,
                    dataType:"json",
                    success:function (data) {
                        if(data.success){
                            $.messager.alert("温馨提示", data.msg);
                ...
                /*$.get("/product/updateProductState/" + rowData.id, function (data) {
    ...
    $("#secondCategory").combobox({
        ...
        panelHeight: 'auto',
        editable: false,
        url: "/secondCategory/AllSecondCategory",
        textField: 'name',
        valueField: 'id',
